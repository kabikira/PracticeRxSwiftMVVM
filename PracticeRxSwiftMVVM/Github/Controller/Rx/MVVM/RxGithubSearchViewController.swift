...
class RxGithubSearchViewController: UIViewController {
    ...
    @IBOutlet private weak var soryTypeSegmentedControl: UISegmentedControl!
    ...
    private func bindInputStream() {
        ...
        let searchTextObservable = urlTextField.rx.text
            .debounce(RxTimeInterval.milliseconds(500), scheduler: MainScheduler.instance).distinctUntilChanged().filterNil().filter { $0.isNotEmpty }
        //ソートのストリーム (2)
        //初回読み込み時または変化があれば
        //sortTypeSegmentedControlのindexをストリームに流す
        let sortTypeObservable = Observable.merge(
            Observable.just(soryTypeSegmentedControl.selectedSegmentIndex),
            soryTypeSegmentedControl.rx.controlEvent(.valueChanged).map{self.soryTypeSegmentedControl.selectedSegmentIndex }
        ).map{ $0 == 0 }
...
extension RxGithubSearchViewController: UITableViewDataSource {
